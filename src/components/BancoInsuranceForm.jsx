...
import { formatCurrencyInput, validateName, validateAmount, validateTerm } from '../utils/validators'

function BancoInsuranceForm({ onClose, onSubmit }) {
  ...
  const calculateMonthlyPayment = () => {
    if (formData.amount && formData.termMonths) {
      ...
      if (!isNaN(amount) && !isNaN(termMonths) && termMonths > 0) {
        return (amount / termMonths).toFixed(2)
      }
    }
    return '0.00'
  }
  ...
  const handleSubmit = (e) => {
    ...
    const amountValidation = validateAmount(formData.amount, 'monto del seguro')
    ...
    <div className="fixed inset-0 bg-black bg-opacity-50 flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-lg shadow-lg max-w-md w-full p-6">
        <div className="flex items-center justify-between mb-6">
          <h2 className="text-xl font-bold text-gray-900">Nuevo Seguro</h2>
          <button
            onClick={onClose}
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          <div>
            ...
            <input
              type="text"
              placeholder="Ej: Seguro Automotriz"
              value={formData.name}
          ...
          {formData.amount && formData.termMonths && !errors.amount && !errors.termMonths && (
            <div className="bg-blue-50 rounded-lg p-3 border border-blue-200">
              <p className="text-sm text-gray-700">
                Pago mensual: <span className="font-bold text-blue-700">${calculateMonthlyPayment()}</span>
              </p>
            </div>
          )}
...
export default BancoInsuranceForm
